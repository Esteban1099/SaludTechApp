...
from src.procesamiento_imagen.modulos.procesamiento_imagen.aplicacion.dto import DemografiaDTO, AtributoDTO, DiagnosticoDTO, \
    RegionAnatomicaDTO
from src.procesamiento_imagen.modulos.procesamiento_imagen.infraestructura.schema.v1.comandos import DemografiaRecord, AtributoRecord, \
    DiagnosticoRecord, RegionAnatomicaRecord, ComandoProcesarImagenMedica, ComandoProcesarImagenMedicaPayload, \
    ComandoEliminarImagenMedica, ComandoEliminarImagenMedicaPayload
from src.procesamiento_imagen.seedwork.infraestructura.mapeadores import Mapeador as InfMap


class MapeadorComandoProcesarImagenMedica(InfMap):
    ...
    def comando_a_comando_integracion(self, comando: ProcesarImagenMedica) -> ComandoProcesarImagenMedica:
        return ComandoProcesarImagenMedica(
    ...
    def comando_integracion_a_comando(self, comando_integracion: ComandoProcesarImagenMedica) -> ProcesarImagenMedica:
        return ProcesarImagenMedica(
            id=comando_integracion.data.id,
            url=comando_integracion.data.url,
            flag=comando_integracion.data.flag,
...
class MapeadorComandoEliminarImagenMedica(InfMap):

    def comando_a_comando_integracion(self, comando: EliminarImagenMedica) -> ComandoEliminarImagenMedica:
        return ComandoEliminarImagenMedica(
            data=ComandoEliminarImagenMedicaPayload(
                id=comando.id
            )
        )

    def comando_integracion_a_comando(self, comando_integracion: ComandoEliminarImagenMedica) -> EliminarImagenMedica:
        return EliminarImagenMedica(
            id=comando_integracion.data.id
        )