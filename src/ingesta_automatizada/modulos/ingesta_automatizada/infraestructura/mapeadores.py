from src.ingesta_automatizada.modulos.ingesta_automatizada.dominio.entidades import Demografia, Diagnostico, \
    RegionAnatomica, Atributo, \
    ImagenMedica
from src.ingesta_automatizada.modulos.ingesta_automatizada.infraestructura.dto import DemografiaDTO, DiagnosticoDTO, \
    RegionAnatomicaDTO, \
    AtributoDTO, ImagenMedicaDTO
from src.ingesta_automatizada.seedwork.dominio.mapeadores import Mapeador
...
class MapeadorImagenMedicaDTOEntity(Mapeador):
    def _procesar_demografia(self, demografia: Demografia) -> DemografiaDTO:
        ...
        demografia_dto.grupo_edad = str(demografia.grupo_edad.value)
        demografia_dto.sexo = str(demografia.sexo.value)
        demografia_dto.etnicidad = str(demografia.etnicidad.value)
    ...
    def _procesar_regiones_anatomicas(self, regiones_anatomicas: list[RegionAnatomica]) -> list[
        ...
        for region_anatomica in regiones_anatomicas:
            ...
            region_anatomica_dto.categoria = str(region_anatomica.categoria.value)
    ...
    def _procesar_demografia_dto(self, demografia_dto: DemografiaDTO) -> Demografia:
        ...
        demografia.id = demografia_dto.id
        demografia._id = demografia_dto.id
    ...
    def _procesar_diagnostico_dto(self, diagnostico_dto: DiagnosticoDTO) -> Diagnostico:
        ...
        diagnostico.id = diagnostico_dto.id
        diagnostico._id = diagnostico_dto.id
    ...
    def _procesar_regiones_anatomicas_dto(self, regiones_anatomicas_dto: list[RegionAnatomicaDTO]) -> list[
        ...
        for region_anatomica_dto in regiones_anatomicas_dto:
            ...
            region_anatomica.id = region_anatomica_dto.id
            region_anatomica._id = region_anatomica_dto.id
    ...
    def _procesar_atributos_dto(self, atributos_dto: list[AtributoDTO]) -> list[Atributo]:
        ...
        for atributo_dto in atributos_dto:
            atributo = Atributo()
            atributo.id = atributo_dto.id
            atributo._id = atributo_dto.id
    ...
    def entidad_a_dto(self, imagen_medica: ImagenMedica) -> ImagenMedicaDTO:
        ...
        imagen_medica_dto.modalidad = str(imagen_medica.modalidad.value)
        imagen_medica_dto.url = imagen_medica.url
        imagen_medica_dto.flag = imagen_medica.flag
